      ...
      layout_column_wrap(
        ...
        card(
          ...
          card_body(
            ...
            tags$p(
              "This Shiny application was developed to explore the
              TCGA Diagnostic Image Database with the possibility to
              extract the R code to download diagnostic images
              features extract from different tools.",
              tags$br(),
...
output$data_table <- renderDT({
  data_with_links <- filtered_data() %>%
    mutate(Data.Type = paste0(
      ...
      image-viewer/MultipleImageViewerPage?caseId=',
      bcr_patient_uuid,
      '" target="_blank">',
      Data.Type,
      '</a>'
...
#     req(selected_rows())
#
#     file_ids <- paste(sprintf('"%s"', selected_images()$File.ID),
#     collapse = ",\n  ")
#
